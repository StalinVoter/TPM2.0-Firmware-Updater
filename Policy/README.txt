...
that state, V0.8 refuses to use tpm20-platformpolicy. V9.3 target evidence showed
...
V0.8 accepts the [POLICYOR_TPMFWUPDATE] section with 2 through 8 contiguous,
...
Before enabling a firmware-update checklist action, V0.8 computes:

  SHA256(zeroDigest || TPM_CC_PolicyOR || PolicyDigest1 || ... || PolicyDigestN)

using TPM wire byte order. V0.8 requires an exact digest match with the live
hierarchy authPolicy. If the TPM cannot report that policy, V0.8 rejects the
